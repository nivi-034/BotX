...
DOCS_CACHE: Dict[str, List[Document]] = {}

def get_docs(data_path: str) -> List[Document]:
    if data_path in DOCS_CACHE:
        return DOCS_CACHE[data_path]
    docs = load_and_chunk_texts(data_path)
    DOCS_CACHE[data_path] = docs
    return docs

# -------- Main Function --------
def ask_question(question: str, data_path: str = None, top_k: int = 8, print_context: bool = False, stream: bool = False):
    """
    Ask a question and get an answer using the RAG system.
    
    Args:
        question (str): The question to ask
        data_path (str): Path to data files (uses env var TXT_PATH if None)
        top_k (int): Number of top results to retrieve
        print_context (bool): Whether to print retrieved context
        stream (bool): Whether to stream the response
        
    Returns:
        str or generator: The answer to the question (string if not streaming, generator if streaming)
    """
    if data_path is None:
        data_path = os.getenv("TXT_PATH", r"./data/*.txt")
    
    # -------- Chunk + prepare IDs/metadata --------
    print(f"Loading & chunking TXT dataset from {data_path} …")
    ...
    if not merged:
        not_found_msg = "Not mentioned in the document."
        if stream:
            yield not_found_msg
            return
        else:
    ...
    if stream:
        ...
        yield answer if answer else "Not mentioned in the document."

# -------- CLI Execution --------
if __name__ == "__main__":
    # Example usage for testing
    res = ask_question("who is jaya mathuri", "e:/echoBot/base/agents/remo.txt", top_k=8, stream=False)
    print("\n================= ANSWER =================")
    print(res)
    print("==========================================")